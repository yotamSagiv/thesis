...
def infer_ols_params(X, Y, has_intercept = True):
	if has_intercept:
		X = sm.add_constant(X, has_constant='add')
...
def expected_discounted_reward(grad, intercept, V, P, gamma, t_0, T, max_tasks, task_dist):
	DISCOUNT_SCALE_FACTOR = 0.1
	s = 0
	for t in range(t_0, T + 1): 
		# if t < 100:
		# 	print("%d: %f, %f, %f, %f" % (t, s, gamma ** (t - t_0), expected_trial_reward(grad, intercept, V, P, t, max_tasks, task_dist), gamma ** (t - t_0) * expected_trial_reward(grad, intercept, V, P, t, max_tasks, task_dist)))
...
eta = 0.2                         # recency weighting parameter
...
for t in np.arange(0, num_trials):
	...
	# basis_intercept = tensor_intercept = 0
	# basis_grad = 1/200
	# tensor_grad = 1/200
	...
	if b_X:
		basis_intercept, basis_grad = infer_wls_params(b_X, b_Y, has_intercept = False, eta = eta)
	if t_X:
		tensor_intercept, tensor_grad = infer_wls_params(t_X, t_Y, has_intercept = False, eta = eta)
	# basis_intercept = tensor_intercept = 0
	# basis_grad = 1/200
	# tensor_grad = 1/200
